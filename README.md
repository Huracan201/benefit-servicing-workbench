...
- **Bounded blast radius.** Reads are deny-by-default (a no-role account sees nothing); every write is authorized server-side by Django (the UI role gate is affordance only); `/internal` is OIDC-gated and unreachable from the public internet; the command endpoints are rate-throttled; Cloud Run is capped (`MAX_INSTANCES`) and Firestore stays in the free tier.
