...
This is the spec-driven **BenefitServicing Workbench** — an operations platform for servicing employer-sponsored student-loan repayment benefits. **All six delivery phases are built, QA'd, and merged to `main`** (each CI-green + CodeRabbit-reviewed). Phases 1–3 — the framework-free financial core, the full command layer, the async layer (PRs #1–#3, #5) + a read-only security review + hardening (PR #4). **Phase 4** — the Workbench UI: design system + dashboard + loan portfolio (PR #6) and the loan/benefit detail + payment/exception worklists + emulator auth + Playwright e2e (PR #7). **Phase 5** — the Phase 3+4 adversarial security review (no CRITICAL/HIGH/MED) + hardening (`If-Match` enforcement, security headers, correlation-id sanitization, money-path logging), PR #8, plus a later integrity pass that made every hardening claim true and bounded the payoff-cancel schedule length (PR #10). **Phase 6 — deployment** (PR #11): `backend/Dockerfile` (Cloud Run), the `/readiness` config flip, `make demo` (one-command local stack), and the [specs/21 §21.4](specs/21-deployment-and-operations.md) runbook as idempotent `bash`+`gcloud` IaC (`infrastructure/` — queues, scheduler, IAM, deploy, teardown), cost-tuned to scale-to-zero. **All six delivery phases are complete AND the system is deployed live** — Cloud Run (project `bsw-demo`, region us-east4) + Vercel + real Firestore/Auth, seeded, gated behind a free shared-password edge-middleware. The live deploy also **hardened the IaC** (the prod-guardrail env, a missing Firestore index, `secretAccessor`, firebase `--force`, the deterministic-URL fix, the access-gate middleware) — full runbook + the gaps it exposed are in [`specs/engineering-reports/deployment-devops.md`](specs/engineering-reports/deployment-devops.md). Only the deferred `propagate-denormalized` fan-out (`U13`, awaiting its producer command) remains.
...
- `.github/workflows/ci.yml` + `.spectral.yaml` — CI: always-on `openapi-lint` + `firestore-rules` jobs, plus a `detect` job gating the backend / frontend / e2e / `infra` jobs on file presence — **all seven active** (the emulator step `cd backend` so Django discovers the tests; the `infra` job shellchecks `infrastructure/scripts/*.sh` and runs a real `docker build` of `backend/Dockerfile`).

**The build is complete — Phases 1–6 on `main`.** Deployment (`U12`) shipped in Phase 6 (PR #11): the `infrastructure/` scripts realize the [specs/19 §19.2](specs/19-delivery-and-scope.md) / [specs/21 §21.4](specs/21-deployment-and-operations.md) runbook (queues, scheduler, IAM, deploy, teardown), `/readiness` reports `configured` when cloud dispatch is wired, and `make demo` brings the full stack up locally with the seeded dataset. The runbook has since been **run for real** against project `bsw-demo` (Cloud Run + Vercel + Firestore/Auth, seeded, shared-password-gated), which hardened several script gaps. The only remaining work is the deferred `propagate-denormalized` fan-out (`U13`, awaiting its producer command). See the [phase-6](specs/engineering-reports/phase-6-deployment.md) + [devops deployment](specs/engineering-reports/deployment-devops.md) reports.
...
```bash
# The FULL stack, one command (emulator + seeded data + Django inline + Next.js) — Phase 6.
# Needs Python 3.12 + backend deps, Node 20 + frontend deps, Java 21 + firebase-tools. Ctrl-C stops.
make demo                               # http://localhost:3000 — then follow docs/demo-script.md
