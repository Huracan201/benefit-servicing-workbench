...
- `frontend/` — **Phase 4 part 1** (on `release/phase-4`): the *ledger + control room* design system (RGB-triplet CSS-var tokens + self-hosted `next/font` faces + a ~18-component kit + zero-dependency inline-SVG charts), the typed **command client** (202-poll write path), the app shell, and the **dashboard** + **loan portfolio** screens reading the read models via the client SDK. Part 2 (loan/benefit detail + payment/exception worklists + polish/e2e) pending. Built on the Phase-1 scaffold (App Router + TS + Tailwind, the emulator-aware Firebase client + `useDocument`/`useCollectionPage` hooks, typed enums).
...
Next work is [specs/19 §19.2](specs/19-delivery-and-scope.md): **Phase 4 part 2** — the loan/benefit detail screen wired to the command client (process/retry/suspend/resume/terminate/employment/notes with confirm dialogs + `If-Match` optimistic concurrency), the payment queue + exception workbench (role-gated actions), and a polish + a11y + committed-lockfile + Playwright-e2e pass. Then deploy (`U12`: provision the Cloud Tasks queues + Cloud Scheduler crons, flip readiness to `configured`).
